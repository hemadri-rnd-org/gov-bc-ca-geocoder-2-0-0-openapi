...
func Get_sites_near_outputformatHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/sites/near.%s%s", cfg.BaseURL, outputFormat, queryString)
...
func CreateGet_sites_near_outputformatTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_sites_near_outputFormat",
		mcp.WithDescription("Find sites near to a geographic point"),
		...
		mcp.WithString("point", mcp.Required(), mcp.Description("The point (x,y) from which the nearby sites will be identified. The coordinates must be specified in the same SRS as given by the 'outputSRS' parameter.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_sites_near_outputformatHandler(cfg),
	}
}
