...
func Get_occupants_occupantid_outputformatHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		args, ok := request.Params.Arguments.(map[string]any)
		if !ok {
			return mcp.NewToolResultError("Invalid arguments object"), nil
		}
		occupantIDVal, ok := args["occupantID"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: occupantID"), nil
		}
		occupantID, ok := occupantIDVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: occupantID"), nil
		}
		...
		url := fmt.Sprintf("%s/occupants/%s.%s%s", cfg.BaseURL, occupantID, outputFormat, queryString)
...
func CreateGet_occupants_occupantid_outputformatTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_occupants_occupantID_outputFormat",
		mcp.WithDescription("Get an occupant (of a site) by its unique ID"),
		mcp.WithString("occupantID", mcp.Required(), mcp.Description("Occupant identifier")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_occupants_occupantid_outputformatHandler(cfg),
	}
}
