...
func Get_intersections_near_outputformatHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/intersections/near.%s%s", cfg.BaseURL, outputFormat, queryString)
...
func CreateGet_intersections_near_outputformatTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_intersections_near_outputFormat",
		mcp.WithDescription("Find intersections near to a geographic point"),
		...
		mcp.WithNumber("outputSRS", mcp.Required(), mcp.Description("The EPSG code of the spatial reference system (SRS) to use for output geometries. See <a href=https://github.com/bcgov/ols-geocoder/blob/gh-pages/glossary.md#outputSRS target=\"_blank\">outputSRS</a>")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_intersections_near_outputformatHandler(cfg),
	}
}
