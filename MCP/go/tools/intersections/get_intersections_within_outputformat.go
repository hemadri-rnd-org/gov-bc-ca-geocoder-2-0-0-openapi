...
func Get_intersections_within_outputformatHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/intersections/within.%s%s", cfg.BaseURL, outputFormat, queryString)
...
func CreateGet_intersections_within_outputformatTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_intersections_within_outputFormat",
		mcp.WithDescription("Find intersections in a geographic area"),
		...
		mcp.WithString("bbox", mcp.Required(), mcp.Description("A bounding box (xmin,ymin,xmax,ymax) used to limit the search area. See <a href=https://github.com/bcgov/ols-geocoder/blob/gh-pages/glossary.md#bbox target=\"_blank\">bbox</a>")),
		...
		mcp.WithNumber("maxResults", mcp.Description("The maximum number of search results")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_intersections_within_outputformatHandler(cfg),
	}
}
